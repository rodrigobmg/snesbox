...
namespace Snes
{
    partial class SMPCore
    {
        public class Flag
        {
            ...
            {
                return (uint)((Convert.ToInt32(flag.n) << 7) + (Convert.ToInt32(flag.v) << 6) + (Convert.ToInt32(flag.p) << 5) + (Convert.ToInt32(flag.b) << 4)
                    + (Convert.ToInt32(flag.h) << 3) + (Convert.ToInt32(flag.i) << 2) + (Convert.ToInt32(flag.z) << 1) + (Convert.ToInt32(flag.c) << 0));
            }
            ...
            public uint Assign(byte data)
            {
                ...
                p = Convert.ToBoolean(data & 0x20);
                b = Convert.ToBoolean(data & 0x10);
                h = Convert.ToBoolean(data & 0x08);
